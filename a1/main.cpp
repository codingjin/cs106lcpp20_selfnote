...
void parse_csv(std::string &&filename, std::vector<Course> &courses) {
  ...
  if (ifs.is_open()) {
    ...
    while (std::getline(ifs, line)) {
      ...
      courses.push_back(Course{record.at(0), record.at(1), record.at(2)});
      //std::cout << "I get title=" << record.at(0) << ", units=" << record.at(1) << ", quarter=" << record.at(2) << std::endl;
    }
  }else {
...
void write_courses_offered(std::vector<Course> &all_courses) {
  ...
  std::vector<Course> to_delete_courses;

  for (const auto &record :all_courses) {
    if (record.quarter != "null") {
      to_delete_courses.push_back(record);
      ofs << record.title << "," << record.number_of_units << "," << record.quarter << "\n";
    }
  }

  for (const auto &record : to_delete_courses) {
    delete_elem_from_vector(all_courses, record);
  }
}
...
void write_courses_not_offered(std::vector<Course> &unlisted_courses) {
  ...
  for (const auto &course : unlisted_courses) {
    ofs << course.title << "," << course.number_of_units << ",null\n";
  }
}
